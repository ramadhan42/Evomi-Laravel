...
<div x-data="evomiAdminMessages" class="space-y-6 pb-12">
    ...
    <div x-show="!loading && !error" class="grid grid-cols-1 lg:grid-cols-3 gap-4 min-h-[30rem]">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-[0_2px_20px_rgb(0,0,0,0.04)] flex flex-col overflow-hidden">
            <div x-show="!selectedEmail" class="flex-1 flex items-center justify-center text-sm text-gray-400 p-8 text-center min-h-[30rem]" x-text="t('messages','pick_user')"></div>
            <div x-show="selectedEmail" x-cloak class="flex flex-col h-full min-h-[30rem]">
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-3 max-h-[26rem] bg-gray-50/30" x-ref="threadPane">
                    ...
                    <template x-for="m in thread" :key="m.id">
                        <div class="rounded-2xl px-4 py-3 text-sm max-w-[85%] shadow-sm" :class="m.type === 'admin' ? 'ml-auto bg-gray-900 text-white' : 'bg-white text-gray-800 border border-gray-100'">
                            <p x-show="m.subject" class="text-[10px] font-semibold opacity-70 mb-1" x-text="m.subject"></p>
                            <p class="whitespace-pre-wrap break-words leading-relaxed" x-text="threadText(m)"></p>
                            <p class="text-[10px] mt-1 opacity-60" x-text="formatTime(m.created_at || m.createdAt)"></p>
                        </div>
                    </template>
