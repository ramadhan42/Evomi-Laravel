...
@section('title', 'Promo | Evomi Admin')

@section('content')
<div x-data="evomiAdminPromos" class="space-y-6 pb-12">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight" x-text="t('promos','title')"></h1>
            <p class="text-gray-500 mt-1 text-sm" x-text="t('promos','subtitle')"></p>
        </div>
    ...
    <div x-show="!loading && !error" class="admin-table-card">
        <div class="p-5 sm:p-6 border-b border-gray-100">
            <div class="relative w-full max-w-md">
                ...
                <input type="search" x-model="search" :placeholder="t('promos','search_ph')" class="admin-search-input">
            </div>
        </div>
            ...
            <table class="w-full border-collapse min-w-[720px]">
                <thead>
                    <tr class="bg-gray-50/80 border-b border-gray-100">
                        <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-center" x-text="t('promos','col_amount')"></th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-center" x-text="t('promos','col_start')"></th>
                        <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-center" x-text="t('promos','col_end')"></th>
                ...
                <tbody class="divide-y divide-gray-50">
                    <template x-if="pagedItems().length === 0">
                        <tr><td colspan="5" class="px-6 py-14 text-center text-sm text-gray-400" x-text="t('promos','empty')"></td></tr>
                    </template>
                    <template x-for="p in pagedItems()" :key="p.id">
                        <tr>
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex px-2.5 py-1 rounded-lg text-[11px] font-bold uppercase tracking-wider border" :class="isActive(p) ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gray-100 text-gray-500 border-gray-200'" x-text="isActive(p) ? t('promos','active') : t('promos','inactive')"></span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center gap-2">
                                    <button type="button" class="admin-btn-icon" @click="openEdit(p)" :title="common().edit"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg></button>
                                    <button type="button" class="admin-btn-icon admin-btn-icon--danger" @click="remove(p.id)" :title="common().delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/></svg></button>
                                </div>
                            </td>
                        </tr>
                    </template>
        ...
        @include('partials.admin-pagination', ['countExpr' => "filteredItems().length + ' ' + t('promos','items')"])
    </div>
    ...
    >
        ...
        >
            <div class="admin-modal-panel__header">
                <h2 class="text-lg font-bold text-gray-900" x-text="modalMode === 'add' ? t('promos','add') : t('promos','edit')"></h2>
            ...
            <form @submit.prevent="save" class="flex flex-col flex-1 min-h-0">
                <div class="admin-modal-panel__body space-y-4">
                    ...
                    <label class="block">
                        <span class="admin-field-label" x-text="t('promos','start_date')"></span>
                        <input type="date" required x-model="form.tanggal_berlaku_promo" class="admin-field-input">
                    </label>
                    <label class="block">
                        <span class="admin-field-label" x-text="t('promos','end_date')"></span>
                        <input type="date" required x-model="form.tanggal_berakhir_promo" class="admin-field-input">
                    </label>
                </div>
