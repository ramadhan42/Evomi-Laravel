        ...
        <div class="relative z-10 w-full max-w-5xl grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 md:gap-6 px-3 sm:px-4 py-5 mt-1">
            @foreach ($products as $index => $product)
                @php
                    ...
                    $soft = $product['soft_accent'] ?? '#9CD6FF';
                @endphp
                <a
                    href="{{ route('belanja.show', $product['id']) }}"
                    class="belanja-card group font-nohemi relative rounded-[16px] md:rounded-[24px] shadow-sm hover:shadow-xl transition-all duration-300 ease-out overflow-hidden flex flex-col border-2 cursor-pointer {{ $index % 2 === 0 ? 'belanja-card--tilt-right' : 'belanja-card--tilt-left' }}"
                    style="border-color: {{ $accent }}"
                    data-soft-nav
                >
                    <div class="relative w-full md:h-[240px] aspect-square overflow-hidden" style="background-color: {{ $accent }}">
                        <span class="absolute top-2 left-2 md:top-5 md:left-5 bg-white px-2 py-1 rounded-full text-[10px] font-bold z-20" style="color: {{ $accent }}">
                            {{ $product['badge'] }}
                        </span>
                    ...
                    <div class="p-2.5 sm:p-3 md:p-4 flex flex-col flex-grow text-left" style="background-color: {{ $soft }}">
                        <h3 class="text-[12px] sm:text-[13px] md:text-[16px] font-bold mb-1 md:mb-2" style="color: {{ $accent }}">
                            {{ $product['title'] }}
                        </h3>

                        <p class="text-[10px] font-medium mb-3 md:mb-2 leading-[1.2] md:leading-[1.25] flex-grow line-clamp-3" style="color: {{ $accent }}; opacity: 0.72">
                            {{ $product['description'] }}
                        </p>

                        <div class="flex justify-between items-center mt-auto gap-1">
                            <span class="text-[11px] md:text-[10px] font-bold" style="color: {{ $accent }}">
                                {{ $product['price_label'] }}
                            </span>

                            <span
                                class="w-7 h-7 rounded-full flex justify-center items-center text-white transition-transform group-hover:scale-105"
